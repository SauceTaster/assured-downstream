...
- the evidence-candidate input shape completes only with locally consistent
  ...
  tooling, and workflow-risk documents; it grants no assurance.
- the production `evaluate-release --target Attested` path remains fail-closed
  until verification results are generated by a code-anchored Sigstore verifier.
- A synthetic external-builder result traverses Build, Trace, Attestation, and
  Governor agents without executing upstream code or mutating GitHub.
